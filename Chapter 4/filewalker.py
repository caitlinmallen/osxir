...
class FileWalker(objkcet):
    def __init__(self, startDir, dumpdir, md5Bool, whitelist):
    if not os.path.isdir(dumpdir):
        os.makedirs(dumpdir)
    self.fileInfo = "%s/fileinfo.txt" % (dumpdir)
    self.fileTimeline = "%sfiletimeline.txt" % (dumpdir)
    self.startDir = startDir
    self.md5Bool = md5Bool
    self.errors = []
    self.fileTypes = {"010":"file" , "014":"socket" , "012":"link" , "060":"block dev" , "004":"dir" , "020":"char dev" , "001":"FIFO"}
    self.specialbits = {"0":"None" , "2048":"SETUID" , "1024":"SETGID" , "512":"STICKYBIT" , "3072":"SETUID/SETGID" , "2560":"SETUID/STICKYBIT" , "1536":"SETGID/STICKYBIT" , "3584":"SETUID/SETGIT/STICKYBIT"}
    ...
    def collect(self, fileName, fi_fd, time_fd):
        try:
            ...
            #check for special file bits - stickybit should not be found since we are not collecting directories 
            x = (mode & stat.S_ISUID) + (mode & stat.S_ISGID) + (mode & stat.S_ISVTX)
            special = self.specialbits[str(x)]
            #create file data for separate file 
...
if __name__ == '__main__':
    ...
    parse.add_argument("-d", "--dumpdir", required=True, help="Specify a directory to store the created files")
