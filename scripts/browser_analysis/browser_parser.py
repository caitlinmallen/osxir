...
def parseChromeHistory(histFile):
    ...
    query = "'"
        SELECT
            strftime('%Y-%m-%dT%H:%M:%S', (d.start_time/1000000)-11644473699, 'unixepoch'),
            dc.url,
            d.target_path, 
            d.danger_type,
            d.opened
...
query = "'"
    ...
    FROM 
        downloads d 
    INNER JOIN 
        downloads_url_chains dc ON dc.id = dc.id;"'"
for row in c.execute(query):
