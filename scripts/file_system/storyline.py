    ...
    for fileName in glob.glob(fileWildCarded):
        with open(fileName) as f:
            for line in f:
                ...
                ts = line.split()[:3]
                ts = ' '.join(ts)
                #add the current year to the syslog because it does not contain it 
                ts = "%s %s" % (now.year, ts)
                #grab rest of the log 
                info = line.split()[4:]
                info = ' '.join (info)
                try:
                    t = time.strptime(ts, "%Y %b %d %H:%M:%S")
                    logEntry = "%s, %s, %s\n" % (time.strftime("%Y-%m-%dT%H:%M:%S", t), title, into)
                    print logEntry
...
def timelineQuarantine(fileName):
    ...
    c = conn.cursor()
    #EventIdentifier, TimeStamp, AgentBundleIdentifier, Name, DataURLString, SenderName SenderAddress TypeNumber OriginTitle OriginURLString LSQuarantineOriginalAlias 
